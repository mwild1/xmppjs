	...
	this.parser = new xml.SaxParser(function (cb) {
		cb.onStartElementNS(function (tagname, attr_arr, prefix, uri, namespaces) {
			...
			if (!stanza)	{
				if (tagname == "error") {
					if (stream.opened)
						callbacks.error("Authentification failed.");
					else
...
exports.Connection.prototype = {
	...
	_socket_connected: function () {
		this.info("CONNECTED.");
		this.send("<stream:stream xmlns='"+xmpp.xmlns.component_accept+"' xmlns:stream='"+xmpp.xmlns.streams+"' to='"+this.jid+"'>");
	},
	
	_socket_disconnected: function (had_error) {
		if (this.status == xmpp.Status.CONNECTING)
			this._setStatus(xmpp.Status.CONNFAIL);
		else if (this.status == xmpp.Status.CONNECTED)
	...
	_handle_stanza: function (stanza) {
		if (!stanza.attr.xmlns) {
			if(stanza.toString() == "<handshake/>") {
	...
	{
		if (this.status == xmpp.Status.AUTHENTICATING)
	...
	if (level === xmpp.LogLevel.DEBUG)
 	    console.log("XMPP.js DEBUG: " + message);
	else if (level === xmpp.LogLevel.INFO)
 	    console.log("XMPP.js INFO: " + message);
	else if (level === xmpp.LogLevel.WARN)
 	    console.log("XMPP.js WARN: " + message);
	else if (level === xmpp.LogLevel.ERROR)
 	    console.error("XMPP.js: " + message);
